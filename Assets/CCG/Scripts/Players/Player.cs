...
[Serializable]
public class Player : Entity    //سكربت اللعب 
{
    //اسم اللعب 
    [Header("Player Info")]
    ...
    public int mana
    {
        //_mana, maxMana ارجاع القية لاصغر بين 
        get { return Mathf.Min(_mana, maxMana); }
        // 0, maxMana حصر القية الناتج بين 
    ...
    // تستدع هذه الدالة عند لاتصال بالخادم للعب المحلي فقط وتستدع مرة واحدة 
    public override void OnStartLocalPlayer()
    {
        localPlayer = this;
        // PlayerPrefs حفض لاسم المدخل من قبل اللعب   
    ...
    {
        //عند تحديث لاسم من لانسبكتر يتم تحديث لاسم ومزامنته مع جميع اللعبين 
        // Update username
        username = newUser;
        //عند تحديث لاسم يتم ايضا تحديث اسم كائن اللعب 
    ...
    {
        //// ابحث عن جميع اللاعبين وأضفهم إلى القائمة.
        ...
        {
            // تأكد من تحميل اللاعبين بشكل صحيح (نقوم بتحميل أسماء المستخدمين أولا)
            // Make sure the players are loaded properly (we load the usernames first)
            if (players.username != "")
            {
                // يجب أن يكون هناك لاعب واحد فقط على الإنترنت ، لذلك إذا لم يكن نحن فهو العدو.
